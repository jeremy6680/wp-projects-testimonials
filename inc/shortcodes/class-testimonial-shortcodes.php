...
class Testimonial_Shortcodes {
    ...
    public function testimonial_shortcode( $atts ) {
        ...
        $author_picture = wp_get_attachment_image( get_post_meta( $testimonial->ID, 'media_picture', true ), 'thumbnail','',["class" => "avatar","alt"=>"image"] );
        $testimonial_text = get_post_meta( $testimonial->ID, 'textarea_text', true );

        // Build the testimonial card HTML
        $html = '<div class="card client-quote container bg-secondary">';
